...
execute if score startSignState lobby matches 1 as @e[tag=startSignMarker] at @s run data merge block ~ ~ ~ {Text1:'{"text":"All players","clickEvent":{"action":"run_command","value":"trigger startSignClick"},"bold":true,"color":"aqua"}',Text2:'{"text":"must","bold":true,"color":"aqua"}',Text3:'{"text":"choose their team.","bold":true,"color":"aqua"}'}
